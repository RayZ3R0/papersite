...
export default function ProfileStats({ subjects, studyPreferences }: ProfileStatsProps) {
  const totalUnits = subjects.reduce((acc, subject) => 
    acc + subject.units.length, 0);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="p-4 bg-surface rounded-lg">
        <h3 className="text-sm text-text-muted mb-2">Subjects</h3>
        <p className="text-2xl font-semibold">{totalSubjects}</p>
        <div className="mt-1 text-sm text-text-muted">
          {totalSubjects === 0 ? 'No subjects added' : 
            subjects.map(s => subjectsList[s.subjectCode as keyof typeof subjectsList]?.name).join(', ')}
        </div>
      </div>
      ...
      <div className="p-4 bg-surface rounded-lg">
        ...
        <div className="mt-1 text-sm text-text-muted">
          {totalUnits === 0 ? 'No units selected' : 
            `${totalUnits} unit${totalUnits !== 1 ? 's' : ''} across ${totalSubjects} subject${totalSubjects !== 1 ? 's' : ''}`}
        </div>
      </div>
      ...
      <div className="p-4 bg-surface rounded-lg">
        ...
        <p className="text-2xl font-semibold">
          ...
          <span className="text-base font-normal text-text-muted"> hrs/day</span>
        </p>
